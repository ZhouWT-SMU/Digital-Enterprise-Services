...
1. `CompanyController` 接收前端提交的 `Company` JSON，`CompanyService` 保存到内存并将结构化字段压缩为文本、段落、下拉、布尔、文件等 Workflow 支持的输入变量。
2. `DifyClient` 通过 `RestClient` 调用 Dify API：`/workflows/{code}/execute` 与 `/chatflows/{code}/messages`。
3. 匹配模块调用 `/api/companies/match`，后端把已提交企业转换成以“---”分隔的段落文本 `context` 传递给 ChatFlow。
